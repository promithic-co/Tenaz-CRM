...
7. **Model turn, then queueing — with nothing in between** —
   ...
   **one independently-delayed row per part** (`delaySeconds: $index * 2` at
   ...
   `app/Jobs/ProcessIncomingWhatsAppMessageJob.php:272-282`.
...
| `followup_messages` | **No row** | Written only by Path 3. Note the inverse interaction: the persister sets `followup_status` to `inactive` when an inbound lands on a lead with an active follow-up (`app/Services/IncomingConversationPersister.php:272-278`) — Path 1 *suppresses* future follow-up rows, it does not create one. |
...
`conversation_timeline_messages` gains one `inbound` row (`app/Services/IncomingConversationPersister.php:290-301`)
...
reused at `app/Services/IncomingConversationPersister.php:270`.
...
| P1-F06 | Inbound replay at the door is guarded | Meta redelivers the same `<wamid>` | `Cache::add("wamid:{<wamid>}", 1, addDay())` drops the redelivery before dispatch (`app/Http/Controllers/MetaWebhookController.php:180-187`); the persister adds a second, DB-level guard on `provider_message_id` (`app/Services/IncomingConversationPersister.php:98-122`) plus a unique-index race resolution (`:290-319`). Fixed and regression-tested. | `meta.webhook_replay_skipped` log; `whatsapp_persister.duplicate_provider_message` log; no duplicate rows. | **F10** (fixed) |

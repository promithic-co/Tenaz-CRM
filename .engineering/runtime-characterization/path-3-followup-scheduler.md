...
row at all**; **[F19](#f19)'s guardrail half, by asserting the turn's complete event vocabulary is
exactly `followup_started`, `model_called`, `outbound_queued` — `model_called` present proving
`AuditLogMiddleware` fires without `AgentService`, and no `fact_check_*` row present proving the
guardrail does not**; and the late-inbound collision, where a customer message persisted after the
eligibility gates pass does not stop the follow-up from being queued and produces no supersession
evidence.

Both halves of F19 are therefore **measured facts, not claims**. When Phase 62 routes follow-up
through the runtime — adding the `AiRun` row and the fact-check guardrail — this file is **expected
to fail** at both: the `doesntExist()` assertion against `AiRun`, and the exhaustive event-vocabulary
assertion the moment a `fact_check_*` row appears. It must then be rewritten deliberately as part of

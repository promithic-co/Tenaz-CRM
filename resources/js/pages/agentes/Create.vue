...
const initialTemplate = props.templates.find((t) => t.slug === props.default_template) ?? props.templates[0];
const initialSpecialization = props.specializations.find((item) => item.value === props.default_specialization) ?? props.specializations[0];
...
const selectedTemplate = computed(() => props.templates.find((t) => t.slug === form.template_slug) ?? null);
...
const isBulkMode = computed(() => selectedTemplate.value?.mode === 'prospeccao');
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="mx-auto max-w-2xl space-y-5 p-4 lg:p-8">
            <div class="rounded-xl border border-sidebar-border/70 bg-card p-6 dark:border-sidebar-border">
                <h1 class="text-base font-semibold text-foreground">Criar agente</h1>
                <p class="mt-1 text-xs text-muted-foreground">
                    Um agente representa uma persona/configuração operacional vinculada a uma instância WhatsApp.
                </p>

                <!-- Template picker -->
                <div class="mt-6">
                    <p class="mb-1 text-sm font-medium text-foreground">Escolha um modelo</p>
                    <p class="mb-3 text-xs text-muted-foreground">
                        Cada modelo vem com personalidade e saudação prontas para o caso de uso. Você pode ajustar tudo depois.
                    </p>
                        ...
                        >
                            <div class="flex items-start gap-3">
                                ...
                                <div class="min-w-0 flex-1">
                                    ...
                                    <p class="mt-1 text-xs font-medium text-foreground/60 italic">{{ tpl.tagline }}</p>
                    ...
                    <p v-if="form.errors.template_slug" class="mt-1.5 text-xs text-red-500">{{ form.errors.template_slug }}</p>
                </div>
                ...
                <div class="mt-5">
                    ...
                    <div class="relative">
                        <button
                            type="button"
                            class="flex w-full items-center justify-between gap-3 rounded-lg border border-input bg-background px-3 py-2 text-left text-sm text-foreground transition-colors hover:bg-muted/40 focus:outline-none focus:ring-2 focus:ring-primary/50"
                            @click="specializationPickerOpen = !specializationPickerOpen"
                        >
                            <span class="min-w-0">
                                <span class="flex items-center gap-2">
                                    ...
                                    <span class="truncate text-xs text-muted-foreground">{{ selectedSpecialization?.description }}</span>
                                </span>
                            </span>
                        ...
                        >
                            ...
                            >
                                <span class="mt-0.5 flex size-4 shrink-0 items-center justify-center rounded border border-border">
                                    <Check v-if="form.agent_niche === specialization.value" class="h-3 w-3 text-primary" />
                                </span>
                                <span class="min-w-0">
                                    <span class="block text-sm font-medium text-foreground">{{ specialization.label }}</span>
                                    <span class="block text-xs text-muted-foreground">{{ specialization.description }}</span>
                                </span>
                            </button>
                        </div>
                    </div>
                    <p v-if="form.errors.agent_niche" class="mt-1 text-xs text-red-500">{{ form.errors.agent_niche }}</p>
                </div>
                ...
                >
                    <Megaphone class="mt-0.5 h-3.5 w-3.5 shrink-0 text-amber-500" />
                    <p class="text-xs text-amber-600 dark:text-amber-400">
                        Este agente funciona melhor vinculado a uma instância usada em <strong>campanhas de disparo</strong> (Evolution, URA ou discadora).
                    </p>
                </div>

                <!-- Form fields -->
                <form class="mt-6 space-y-4" @submit.prevent="form.post('/agentes')">
                    <div class="grid gap-4 sm:grid-cols-2">
                        <div>
                            ...
                            <input
                                ...
                                class="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/50"
                            />
                    ...
                    <div>
                        <label class="mb-1.5 block text-sm font-medium text-foreground">Descrição (opcional)</label>
                        <input
                    ...
                    <div>
                        <label class="mb-1.5 block text-sm font-medium text-foreground">Instância WhatsApp <span class="text-muted-foreground font-normal">(opcional)</span></label>
                        <select
                            v-model.number="form.whatsapp_instance_id"
                            class="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/50"
                        >
                            <option :value="null">Criar sem vincular WhatsApp</option>
                            <option v-for="instance in instances" :key="instance.id" :value="instance.id">
                                {{ instance.display_name || instance.name }}
                                <template v-if="instance.phone_number"> · {{ instance.phone_number }}</template>
                            </option>
                        </select>
                        <p v-if="form.errors.whatsapp_instance_id" class="mt-1 text-xs text-red-500">
                            {{ form.errors.whatsapp_instance_id }}
                        </p>
                        <p v-if="!instances.length" class="mt-1 text-xs text-amber-600 dark:text-amber-400">
                            Você pode criar o agente agora. Ele ficará inativo até vincular uma instância WhatsApp.
                            <Link href="/whatsapp" class="text-primary underline">Conectar WhatsApp</Link>
                        </p>
                        <p v-else-if="form.whatsapp_instance_id === null" class="mt-1 text-xs text-muted-foreground">
                            O agente ficará inativo até que uma instância WhatsApp seja vinculada.
                        </p>
                    </div>
                    ...
                    <div class="flex items-center justify-end gap-2 pt-2">
                        ...
                        >
                            {{ form.processing ? 'Criando agente...' : 'Criar agente' }}
                        </button>
                    </div>
                </form>
            </div>

...
const entriesPreview = ref<
    {
        cpf: string;
        nome: string;
        status_expected: string;
        has_qualified_json: boolean;
    }[]
>([]);
...
async function submitUpload() {
    ...
    if (!file || !name.value.trim()) {
        uploadMessage.value = {
            type: 'error',
            text: 'Selecione um arquivo e informe o nome.',
        };
        return;
    }
    ...
    try {
        ...
        if (res.ok) {
            uploadMessage.value = {
                type: 'success',
                text: `Importados ${data.data?.total_entries ?? 0} CPFs no dataset "${data.data?.name}".`,
            };
            name.value = '';
            description.value = '';
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-col gap-6 p-3 sm:p-4">
                ...
                <div class="flex flex-wrap items-end gap-3">
                    ...
                    <div class="w-full sm:min-w-[180px] sm:flex-1">
                        ...
                        <input
                            v-model="name"
                            type="text"
                            placeholder="Ex: Exportação Promosys Março"
                            class="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                        />
                    </div>
                    <div class="w-full sm:min-w-[180px] sm:flex-1">
            ...
            >
                    ...
                    <table class="w-full min-w-[44rem] text-sm">
                        ...
                        >
                            ...
                            <tr v-if="!datasets.length">
                                ...
                                >
                                    Nenhum dataset ainda. Importe um arquivo CSV
                                    ou JSON acima.
                                </td>
                            </tr>
                        </tbody>
                ...
                <div
                    v-if="showEntriesId"
                    class="overflow-x-auto border-t border-sidebar-border/70 p-4 dark:border-sidebar-border"
                >
                    <p class="mb-2 text-xs font-medium text-muted-foreground">
                        Primeiros registros (máx. 20)
                    </p>
                    <table class="w-full min-w-[32rem] text-sm">
                        <thead>
                            ...
                            >
                                <th
                                    class="pb-1 font-medium text-muted-foreground"
                                >
                                    CPF
                                </th>
                                <th
                    ...
                    <button
                        type="button"
                        class="mt-2 text-xs text-muted-foreground underline hover:no-underline"
                        @click="showEntriesId = null"
                    >
                        Fechar
                    </button>
                </div>

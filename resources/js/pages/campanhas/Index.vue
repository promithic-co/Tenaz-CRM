...
function startCampaign(id: number): void {
    getActionForm(`start-${id}`).post(CampaignController.start(id).url, { preserveScroll: true });
}

function pauseCampaign(id: number): void {
    getActionForm(`pause-${id}`).post(CampaignController.pause(id).url, { preserveScroll: true });
}

function resumeCampaign(id: number): void {
    getActionForm(`resume-${id}`).post(CampaignController.resume(id).url, { preserveScroll: true });
}
...
<template>
            ...
            <div class="overflow-hidden rounded-xl border border-sidebar-border/70 bg-card dark:border-sidebar-border">
                ...
                <div class="flex items-center justify-between border-b border-sidebar-border/70 px-4 py-3 dark:border-sidebar-border">
                    <div class="flex items-center gap-3">
                        <span class="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Campanhas</span>
                        <span class="rounded-full bg-muted px-2 py-0.5 text-xs text-muted-foreground">{{ campaigns.total }} campanhas</span>
                    </div>
                    <Link
                ...
                <table class="w-full text-sm">
                    <thead class="border-b border-sidebar-border/70 bg-muted/40 dark:border-sidebar-border">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-semibold uppercase text-muted-foreground">Enviados/Total</th>
                            <th class="px-4 py-3 text-left text-xs font-semibold uppercase text-muted-foreground">Taxa Entrega</th>
                    ...
                    <tbody class="divide-y divide-sidebar-border/70 dark:divide-sidebar-border">
                        ...
                        >
                            ...
                            <td class="px-4 py-3 text-xs text-muted-foreground">{{ campaign.whatsapp_template?.name ?? '—' }}</td>
                            <td class="px-4 py-3 text-xs text-foreground">
                                {{ campaign.total_sent ?? 0 }}/{{ campaign.total_recipients ?? 0 }}
                            </td>
                            <td class="px-4 py-3 text-xs font-semibold" :class="deliveryRateClass(deliveryRate(campaign))">
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <!-- Draft / Scheduled -->
                                    <template v-if="campaign.status === 'draft' || campaign.status === 'scheduled'">
                                        <button
                                        ...
                                        <button
                                            ...
                                            @click="deleteConfirmId = campaign.id"
                                        >
                                            Excluir
                                        </button>
                                    </template>
                                    ...
                                    <template v-else-if="campaign.status === 'sending'">
                                        ...
                                        <Link :href="CampaignController.show(campaign.id).url" class="rounded px-2 py-1 text-xs text-muted-foreground transition-colors hover:bg-muted hover:text-foreground">
                                            Ver
                                        </Link>
                                    </template>
                                    <!-- Paused -->
                ...
                <!-- Pagination -->
                <div v-if="campaigns.links?.length > 3" class="flex items-center gap-1 border-t border-sidebar-border/70 px-4 py-3 dark:border-sidebar-border">
    ...
    <Dialog :open="deleteConfirmId !== null" @update:open="(v) => { if (!v) deleteConfirmId = null; }">
        <DialogContent class="sm:max-w-sm">
            ...
            <p class="text-sm text-muted-foreground">Tem certeza que deseja excluir esta campanha? Esta ação não pode ser desfeita.</p>
            <DialogFooter>
                <button
                    type="button"

...
const breadcrumbs: BreadcrumbItem[] = [{ title: 'WhatsApp', href: '/whatsapp' }];
...
function loadFbSdk(): Promise<void> {
    return new Promise((resolve, reject) => {
        ...
        if (!appId) { reject(new Error('META_APP_ID não configurado.')); return; }
...
function launchEmbeddedSignup(): void {
    ...
    if (!metaConfigId()) {
        metaError.value = 'Configuração do Meta (META_APP_CONFIG_ID) não definida no servidor. Contate o suporte.';
        return;
    }
    ...
    loadFbSdk()
        .then(() => openFbLogin())
        .catch((err: any) => {
            metaError.value = err?.message ?? 'Não foi possível carregar o SDK do Facebook. Verifique sua conexão.';
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="space-y-6 p-4 lg:p-8">
            ...
            >
                ...
                <span class="text-xs text-muted-foreground">Conecte uma instância e volte para concluir a configuração.</span>
            </div>
                ...
                <div class="flex items-center gap-2.5">
                    <div>
                        <div class="flex items-center gap-2">
                            <h1 class="text-base font-semibold text-foreground">WhatsApp</h1>
                            <Badge v-if="instances.length" variant="outline" class="tabular-nums">
                                {{ instances.length }}
                            </Badge>
                        </div>
                        <p class="mt-0.5 text-xs text-muted-foreground">Gerencie suas conexões WhatsApp.</p>
                    </div>
                </div>
            ...
            >
                <div class="mx-auto mb-3 flex h-10 w-10 items-center justify-center rounded-full bg-muted">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="text-muted-foreground">
                        <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" />
                    </svg>
                </div>
                <p class="text-sm font-medium text-foreground">Nenhuma instância configurada</p>
                <p class="mt-1 text-xs text-muted-foreground">Clique em "+ Nova instância" para adicionar sua primeira conexão WhatsApp.</p>
    ...
    <!-- ─── Add Instance Dialog ──────────────────────────────────────────────── -->
    <Dialog :open="showAddDialog" @update:open="(v: boolean) => { if (!v) closeDialog() }">
            ...
            <form class="space-y-4 py-2" @submit.prevent="submitAdd">
                ...
                <div class="flex items-center gap-2 rounded-lg border border-border bg-muted/40 px-3 py-2">
                    ...
                    <div class="flex-1 min-w-0">
                        <p class="text-xs font-semibold text-foreground">Conexão via Meta Cloud API</p>
                        <p class="text-[11px] text-muted-foreground">WhatsApp Business Platform oficial da Meta.</p>
                    </div>
                </div>
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <div class="space-y-1.5">
                        <Label>Apelido <span class="text-muted-foreground">(opcional)</span></Label>
                        <Input v-model="addForm.display_name" placeholder="ex: Vendas SP" />
                    </div>

                    <div class="space-y-1.5">
                        <Label>Nome da instância <span class="text-red-500">*</span></Label>
                        <Input v-model="addForm.name" class="font-mono" placeholder="ex: vendas-sp" />
                ...
                <template v-if="metaStage === 'select'">
                    <div class="space-y-2">
                        ...
                        <div class="space-y-2">
                            <button
                                v-for="opt in ([
                                    { value: 'new',         label: 'Número novo',                  desc: 'Ative um número que ainda não usa o WhatsApp Business API. Você receberá um código OTP por chamada ou SMS.' },
                                    { value: 'migrate',     label: 'Migrar número existente',      desc: 'Transfira um número que usa o app WhatsApp Business para a Cloud API. O app original será desativado.' },
                                    { value: 'coexistence', label: 'Coexistência (manter app)',    desc: 'Use a Cloud API e o app WhatsApp Business ao mesmo tempo via QR code. Disponível para BR/MX/IN. Limite: 20 msg/seg.' },
                                ] as const)"
                                :key="opt.value"
                                type="button"
                                :class="[
                            ...
                            >
                                <span class="font-semibold text-foreground">{{ opt.label }}</span>
                                <p class="mt-0.5 text-muted-foreground">{{ opt.desc }}</p>
                            </button>
                        </div>
                    </div>

                    <div v-if="metaError" class="rounded-lg border border-red-200 bg-red-50 px-3 py-2.5 text-xs text-red-700 dark:border-red-900/50 dark:bg-red-900/20 dark:text-red-400">
                        {{ metaError }}
                    </div>
                </template>
                ...
                <template v-if="metaStage === 'processing'">
                    <div class="flex flex-col items-center gap-3 py-6">
                        ...
                        <p class="text-sm text-muted-foreground">Aguardando autorização do Facebook...</p>
                        <p class="text-xs text-muted-foreground">Conclua o fluxo na janela do Facebook que abriu.</p>
                    </div>
                </template>

                <!-- Stage: confirm + optional PIN -->
                <template v-if="metaStage === 'confirm'">
                    <div class="rounded-lg border border-green-200 bg-green-50 px-3 py-2.5 text-xs text-green-700 dark:border-green-900/50 dark:bg-green-900/20 dark:text-green-400 space-y-1.5">
                        <div class="flex items-center gap-2">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
                            <span class="font-semibold">Conta vinculada com sucesso!</span>
                        </div>
                        <div class="grid grid-cols-2 gap-x-4 gap-y-0.5 pl-1">
                            <span class="text-green-600 dark:text-green-500">Modo:</span>
                            <span class="font-medium">{{ metaModeLabel[metaMode] }}</span>
                            <span class="text-green-600 dark:text-green-500">WABA ID:</span>
                            <span class="font-mono font-medium">{{ metaWabaId || '—' }}</span>
                            <span class="text-green-600 dark:text-green-500">Phone Number ID:</span>
                            <span class="font-mono font-medium">{{ metaPhoneNumberId || '—' }}</span>
                        </div>
                    </div>

                    <!-- PIN only for new / migrate modes -->
                    <div v-if="needsPin()" class="space-y-1.5">
                        <Label>PIN de registro <span class="text-muted-foreground">(opcional)</span></Label>
                        <Input
                        ...
                        <p class="text-xs text-muted-foreground">
                            PIN de 6 dígitos para registro do número na Cloud API. Deixe em branco para usar <code>000000</code> (padrão).
                        </p>
                        <p v-if="addForm.errors.meta_pin" class="text-xs text-red-500">{{ addForm.errors.meta_pin }}</p>
                    </div>

                    <p v-if="addForm.errors.meta_signup_token" class="text-xs text-red-500">{{ addForm.errors.meta_signup_token }}</p>
                </template>
                ...
                <DialogFooter>
                    ...
                    <Button
                        ...
                        type="submit"
                        :disabled="addForm.processing || !addForm.meta_signup_token"
                    >
                        {{ addForm.processing ? 'Criando...' : 'Criar instância' }}
                    </Button>
                </DialogFooter>
            </form>
        </DialogContent>
    ...
    <Teleport to="body">
        ...
        >
            <div class="w-full max-w-sm rounded-xl bg-card p-6 shadow-xl">
                <h3 class="text-sm font-semibold text-foreground">Remover instância</h3>
                <p class="mt-2 text-sm text-muted-foreground">
                    Tem certeza? A instância será removida do sistema. O WhatsApp não será desconectado automaticamente.
                </p>
                <div class="mt-4 flex justify-end gap-2">
                    <Button variant="outline" size="sm" @click="confirmDeleteId = null">Cancelar</Button>
                    <Button variant="destructive" size="sm" @click="deleteInstance(confirmDeleteId!)">Remover</Button>
                </div>
            </div>
        </div>
    </Teleport>

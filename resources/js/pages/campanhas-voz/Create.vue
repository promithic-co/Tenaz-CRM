...
const voiceOptions = [
    ...
    { value: 'Polly.Camila-Neural',     label: '⭐ Camila Neural (Feminino) — Amazon Polly' },
    { value: 'Polly.Thiago-Neural',     label: '⭐ Thiago Neural (Masculino) — Amazon Polly' },
    { value: 'Polly.Vitoria-Neural',    label: '⭐ Vitória Neural (Feminino) — Amazon Polly' },
];

// ─── Available DTMF actions ───────────────────────────────────────────────────

const dtmfActionOptions = [
    { value: 'interested', label: 'Tenho interesse — notificar agente via WhatsApp' },
...
function syncDtmfToForm() {
    ...
    for (const row of dtmfRows.value) {
        if (row.digit) map[row.digit] = { action: row.action, label: row.label };
    }
...
function addDtmfRow() {
    ...
    const next = ['1','2','3','4','5','6','7','8','9'].find((d) => !used.has(d)) ?? '';
...
async function generatePreview() {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            previewError.value = (json as { error?: string }).error ?? 'Falha ao gerar áudio.';
            return;
        }
...
const dtmfIsValid = computed(() => dtmfRows.value.length > 0 && dtmfRows.value.every((r) => r.digit && r.action));
</script>
            ...
            <div class="mx-auto max-w-2xl">
                <div class="overflow-hidden rounded-xl border border-sidebar-border/70 bg-card dark:border-sidebar-border">
                    ...
                    <div class="border-b border-sidebar-border/70 px-6 py-4 dark:border-sidebar-border">
                        <h1 class="text-base font-semibold text-foreground">Nova Campanha de Voz</h1>
                        <p class="mt-1 text-xs text-muted-foreground">Configure uma campanha de ligações automáticas via Twilio.</p>
                    </div>
                    ...
                    <div class="flex flex-col gap-5 p-6">
                        ...
                        <div>
                            <label class="mb-1 block text-sm font-medium text-foreground">
                                Lista de Contatos <span class="text-red-500">*</span>
                            </label>
                            <select
                            ...
                            >
                                ...
                                <option v-for="list in contactLists" :key="list.id" :value="list.id">
                                    {{ list.name }} ({{ list.entries_count }} contatos)
                                </option>
                            </select>
                            <p v-if="form.errors.contact_list_id" class="mt-1 text-xs text-red-500">{{ form.errors.contact_list_id }}</p>
                        </div>
                        ...
                        >
                            Número de saída: <span class="font-mono font-medium">{{ props.twilioPhoneNumber }}</span>
                        </div>

                        <!-- Voice instance -->
                        <div>
                            <label class="mb-1 block text-sm font-medium text-foreground">
                                Instância de Voz <span class="text-red-500">*</span>
                            </label>
                            <select
                            ...
                            >
                                ...
                                <option v-for="instance in voiceInstances" :key="instance.id" :value="instance.id">
                                    {{ instance.display_name ?? instance.name }}
                                </option>
                            </select>
                            <p v-if="form.errors.voice_instance_id" class="mt-1 text-xs text-red-500">{{ form.errors.voice_instance_id }}</p>
                            <p v-if="voiceInstances.length === 0" class="mt-1 text-xs text-yellow-600 dark:text-yellow-400">
                                Nenhuma instância de voz ativa. Crie uma em <a href="/voz" class="underline">Instâncias de Voz</a>.
                            </p>
                        </div>

                        <!-- Greeting template -->
                        <div>
                            <label class="mb-1 block text-sm font-medium text-foreground">Texto da URA (o robô irá falar)</label>
                            <textarea
                            ...
                            <p class="mt-1 text-xs text-muted-foreground">
                                Use <span class="rounded bg-muted px-1 font-mono text-xs">{nome}</span>,
                                <span class="rounded bg-muted px-1 font-mono text-xs">{valor}</span> e outros campos do contato.
                            </p>
                            <p v-if="form.errors.greeting_template" class="mt-1 text-xs text-red-500">{{ form.errors.greeting_template }}</p>
                        </div>

                        <!-- TTS Voice selector + Audio preview -->
                        <div>
                            <label class="mb-1 block text-sm font-medium text-foreground">Voz do Robô</label>
                            <div class="flex gap-2">
                                <select
                                    v-model="form.tts_voice"
                                    class="flex-1 rounded-md border border-input bg-background px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-ring"
                                >
                                ...
                                >
                                    <svg v-if="!previewBusy" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-primary" viewBox="0 0 24 24" fill="currentColor">
                                        <path d="M8 5v14l11-7z"/>
                                    </svg>
                                    <svg v-else class="h-4 w-4 animate-spin text-muted-foreground" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
                                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z"/>
                                    </svg>
                                    {{ previewBusy ? 'Gerando...' : 'Ouvir prévia' }}
                                </button>
                            </div>
                            ...
                            <p v-if="previewError" class="mt-1 text-xs text-red-500">{{ previewError }}</p>
                            <p class="mt-1 text-xs text-muted-foreground">
                                ⭐ Vozes Neural (Amazon Polly) têm qualidade superior. O preview usa a mesma voz que seus clientes ouvirão.
                            </p>
                            <p v-if="form.errors.tts_voice" class="mt-1 text-xs text-red-500">{{ form.errors.tts_voice }}</p>
                        </div>
                        ...
                        <div>
                            <div class="mb-2 flex items-center justify-between">
                                <label class="text-sm font-medium text-foreground">
                                    Ações do Teclado (DTMF) <span class="text-red-500">*</span>
                                </label>
                                <button
                            ...
                            <div class="flex flex-col gap-2">
                                ...
                                >
                                    <!-- Digit -->
                                    <div class="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-md border border-input bg-muted text-sm font-bold text-foreground">
                                        {{ row.digit }}
                                    </div>
                                    ...
                                    <select
                                        v-model="row.action"
                                        class="flex-1 rounded-md border border-input bg-background px-2 py-1.5 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-ring"
                                        @change="onActionChange(i)"
                                    >
                                        <option v-for="opt in dtmfActionOptions" :key="opt.value" :value="opt.value">
                                            {{ opt.label }}
                                        </option>
                                    </select>
                                    ...
                                    <input
                                        ...
                                        class="w-40 rounded-md border border-input bg-background px-2 py-1.5 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring"
                            ...
                            <p v-if="dtmfRows.length === 0" class="mt-1 text-xs text-yellow-600 dark:text-yellow-400">
                                ⚠ Adicione pelo menos uma ação DTMF. A campanha não pode ser iniciada sem elas.
                            </p>
                            <p class="mt-1 text-xs text-muted-foreground">
                                O robô lerá as opções em voz alta após o texto da URA. O cliente pressiona o dígito correspondente.
                            </p>
                            <p v-if="form.errors['dtmf_actions']" class="mt-1 text-xs text-red-500">{{ form.errors['dtmf_actions'] }}</p>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea
                                ...
                                class="w-full rounded-md border border-input bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring"
                            />
                            <p class="mt-1 text-xs text-muted-foreground">Mensagem enviada via WhatsApp após o contato expressar interesse na ligação.</p>
                            <p v-if="form.errors.post_call_message" class="mt-1 text-xs text-red-500">{{ form.errors.post_call_message }}</p>
                        </div>

                        <!-- Delay between calls -->
                        <div>
                            <label class="mb-1 block text-sm font-medium text-foreground">Atraso entre ligações (ms)</label>
                            <input
                            ...
                            />
                            <p class="mt-1 text-xs text-muted-foreground">
                                Tempo de espera entre cada ligação. Mínimo: 1000ms (1s). Atual: {{ (form.delay_between_calls_ms / 1000).toFixed(1) }}s.
                            </p>
                            <p v-if="form.errors.delay_between_calls_ms" class="mt-1 text-xs text-red-500">{{ form.errors.delay_between_calls_ms }}</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-between border-t border-sidebar-border/70 px-6 py-4 dark:border-sidebar-border">
                        ...
                        <button
                            ...
                            :title="!dtmfIsValid ? 'Configure pelo menos uma ação DTMF antes de criar.' : undefined"

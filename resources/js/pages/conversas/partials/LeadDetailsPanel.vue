...
const conversationWindow = computed(() => props.conversation.conversationWindow ?? null);

type LeadTag = { id: number; name: string; slug?: string; color?: string | null };
const leadTags = ref<LeadTag[]>([...(props.conversation.lead.tags ?? [])]);
const tagsSyncing = ref(false);

function syncLeadTags(payload: { tag_ids?: number[]; tag_names?: string[] }): void {
...
function updateAiMode(): void {
    aiModeForm.patch(aiMode.url({ lead: lead.value.id }), { preserveScroll: true });
}
...
function submitPrepareCampaign(): void {
    prepareCampaignForm.post(prepareCampaign.url({ lead: lead.value.id }), { preserveScroll: false });
}

function submitClaim(): void {
    claimForm.post(claim.url({ lead: lead.value.id }), { preserveScroll: true });
}
    ...
    <aside class="hidden min-h-0 flex-col gap-2 overflow-y-auto border-l border-sidebar-border/70 bg-card p-3 dark:border-sidebar-border xl:flex">
            ...
            <div class="flex flex-col items-center text-center">
                ...
                <div class="mt-3">
                    <StatusSelect
                        ...
                        :available-transitions="lead.available_transitions ?? []"
                        :lead-id="lead.id"
                    />
                </div>
                <div v-if="leadTags.length > 0" class="mt-3 flex flex-wrap items-center justify-center gap-1">
                    <TagChip v-for="t in leadTags" :key="t.id" :tag="t" />
                </div>
            </div>

            <div class="mt-4">
                <p class="mb-1.5 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">Tags</p>
                <TagInput
                ...
                <form
                    class="mt-2"
                    @submit.prevent="autoTagForm.post(autoTagStore.url({ lead: lead.id }), { preserveScroll: true })"
                >
                    <button
                        type="submit"
                    ...
                    >
                        <RefreshCw class="h-3.5 w-3.5" :class="{ 'animate-spin': autoTagForm.processing }" />
                        Reavaliar com IA
                    </button>
                </form>
            </div>
        ...
        <section class="rounded-lg border border-sidebar-border/70 bg-background/40 p-3 dark:border-sidebar-border">
            <div class="space-y-3 text-sm">
                <div class="flex items-center justify-between gap-3">
                    ...
                    <span class="font-mono text-xs text-foreground">{{ lead.cpf ? formatCpf(lead.cpf) : 'Não informado' }}</span>
                </div>
                <div class="flex items-center justify-between gap-3">
                    <span class="text-muted-foreground">Idade</span>
                    <span class="text-xs text-foreground">{{ lead.idade ? `${lead.idade} anos` : 'Não informada' }}</span>
                </div>
                <div class="flex items-center justify-between gap-3">
                    <span class="text-muted-foreground">Atendente</span>
                    <span class="truncate text-right text-xs text-foreground">{{ lead.assigned_user_name ?? 'Sem atendente' }}</span>
                </div>
                <form v-if="!lead.assigned_user_id" @submit.prevent="submitClaim">
                    <button
                        type="submit"
                ...
                <div class="flex items-center justify-between gap-3">
                    ...
                    <span class="text-right text-xs font-medium text-foreground">{{ aiModeLabels[lead.effective_ai_mode] ?? lead.effective_ai_mode }}</span>
                </div>
                <div class="flex items-center justify-between gap-3">
                    <span class="text-muted-foreground">Etapa</span>
                    <span class="text-right text-xs text-foreground">{{ stageLabels[lead.operational_stage] ?? lead.operational_stage }}</span>
                </div>
            </div>
        </section>
                    ...
                    <span
                        :class="[
                            ...
                            conversationWindow.service_window.status === 'open'
                                ...
                                : conversationWindow.service_window.status === 'closed'
                                    ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400'
                                    : 'bg-muted text-muted-foreground'
                        ]"
                    >
                        {{ conversationWindow.service_window.status === 'open' ? 'Aberta' : conversationWindow.service_window.status === 'closed' ? 'Fechada' : 'Sem dados' }}
                    </span>
                </div>
                <div v-if="conversationWindow.service_window.status === 'open'" class="flex items-center justify-between gap-3">
                    <span class="text-muted-foreground">Restante</span>
                    <span class="text-foreground">{{ formatRemaining(conversationWindow.service_window.remaining_seconds) }}</span>
                </div>
                <div class="flex items-center justify-between gap-3">
                    <span class="text-muted-foreground">Template necessário</span>
                    <span class="text-foreground">{{ conversationWindow.template_required ? 'Sim' : 'Não' }}</span>
                </div>
                <div v-if="conversationWindow.free_entry_point.status !== 'unknown'" class="mt-1 flex items-center justify-between gap-3">
                    ...
                    >
                        {{ conversationWindow.free_entry_point.status === 'active' ? 'Ativa' : 'Expirada' }}
                    </span>
                </div>
                <div v-if="conversationWindow.free_entry_point.status === 'active'" class="flex items-center justify-between gap-3">
                    <span class="text-muted-foreground">Restante</span>
                    <span class="text-foreground">{{ formatRemaining(conversationWindow.free_entry_point.remaining_seconds) }}</span>
                </div>
                <div v-if="conversationWindow.free_entry_point.source" class="flex items-center justify-between gap-3">
                    <span class="text-muted-foreground">Origem</span>
                    <span class="text-foreground">{{ sourceLabels[conversationWindow.free_entry_point.source] ?? conversationWindow.free_entry_point.source }}</span>
                </div>
                <p
                    v-if="conversationWindow.coexistence.enabled && conversationWindow.coexistence.note"
        ...
        >
            ...
            <div class="space-y-3">
                <div class="flex items-center justify-between gap-2">
                    ...
                    <span :class="[
                        'rounded-full px-2 py-0.5 text-[10px] font-medium',
                        conversation.handoff_state === 'waiting_human' ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400' :
                        conversation.handoff_state === 'human_active' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' :
                        conversation.handoff_state === 'waiting_customer' ? 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400' :
                        'bg-muted text-muted-foreground'
                    ]">
                        {{
                            conversation.handoff_state === 'waiting_human' ? 'Aguardando atendimento' :
                            conversation.handoff_state === 'human_active' ? 'Em atendimento humano' :
                            conversation.handoff_state === 'waiting_customer' ? 'Aguardando cliente' :
                            conversation.handoff_state === 'closed' ? 'Encerrado' : 'IA ativa'
                        }}
                    </span>
                </div>
                <template v-if="conversation.active_handoff">
                    <div v-if="conversation.active_handoff.reason" class="flex items-center justify-between gap-2">
                        <span class="text-xs text-muted-foreground">Motivo</span>
                        <span class="text-right text-xs text-foreground">{{ conversation.active_handoff.reason }}</span>
                    </div>
                    <div v-if="conversation.active_handoff.summary" class="text-xs text-muted-foreground">
                        {{ conversation.active_handoff.summary }}
                    </div>
                    <div v-if="conversation.active_handoff.assigned_user_name" class="flex items-center justify-between gap-2">
                        <span class="text-xs text-muted-foreground">Responsavel</span>
                        <span class="text-right text-xs text-foreground">{{ conversation.active_handoff.assigned_user_name }}</span>
                    </div>
                    <div v-if="conversation.active_handoff.sla_due_at" class="flex items-center justify-between gap-2">
                        <span class="text-xs text-muted-foreground">SLA</span>
                        <span :class="['text-right text-xs', conversation.active_handoff.sla_overdue ? 'text-red-500 font-medium' : 'text-foreground']">
                            {{ new Date(conversation.active_handoff.sla_due_at).toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' }) }}
                            <span v-if="conversation.active_handoff.sla_overdue"> (vencido)</span>
                        </span>
                    </div>
                </template>
                <div v-if="conversation.handoff_actions.includes('return_to_ai')" class="pt-1">
                    <form @submit.prevent="returnToAiForm.post(returnToAi.url({ ticket: conversation.active_handoff!.id }), { preserveScroll: true })">
                        <button
                            type="submit"
                ...
                <div v-if="conversation.handoff_actions.includes('keep_manual')">
                    <form @submit.prevent="keepManualForm.post(keepManual.url({ ticket: conversation.active_handoff!.id }), { preserveScroll: true })">
                        <button
                            type="submit"
        ...
        <section class="rounded-lg border border-sidebar-border/70 bg-background/40 p-3 dark:border-sidebar-border">
            ...
            >
                ...
                <option value="qualify_then_handoff">Qualifica e transfere</option>
            </select>
            <form v-if="!conversation.pausado" @submit.prevent="pauseForm.post(pause.url({ lead: lead.id }), { preserveScroll: true })">
                <button
                    type="submit"
            ...
            <form v-else @submit.prevent="resumeForm.post(resume.url({ lead: lead.id }), { preserveScroll: true })">
                <button
                    type="submit"
                ...
                <p class="mt-2 text-center text-xs font-medium text-amber-600">Agente pausado - responda manualmente</p>
            </form>
        </section>
        ...
        <section class="rounded-lg border border-sidebar-border/70 bg-background/40 p-3 dark:border-sidebar-border">
            ...
            <div class="space-y-2">
                <form
                    ...
                    @submit.prevent="pauseFollowUpForm.post(pauseFollowup.url({ lead: lead.id }), { preserveScroll: true })"
                >
                    <button
                        type="submit"
                ...
                <form
                    v-if="followupStatus === 'paused'"
                    @submit.prevent="resumeFollowUpForm.post(resumeFollowup.url({ lead: lead.id }), { preserveScroll: true })"
                >
                    <button
                        type="submit"
                ...
                <form
                    ...
                    @submit.prevent="reactivateFollowUpForm.post(reactivateFollowup.url({ lead: lead.id }), { preserveScroll: true })"
                >
                    <button
                        type="submit"
                ...
                <form
                    v-if="followupStatus !== 'inactive'"
                    @submit.prevent="disableFollowUpForm.post(disableFollowup.url({ lead: lead.id }), { preserveScroll: true })"
                >
                    <button
                        type="submit"
        ...
        <section class="rounded-lg border border-sidebar-border/70 bg-background/40 p-3 dark:border-sidebar-border">
            ...
            <div v-if="conversation.followupHistory.length > 0" class="space-y-3">
                <div v-for="item in conversation.followupHistory" :key="item.attempt" class="border-l-2 border-muted pl-3">
                    <div class="flex items-center justify-between gap-2">
                        <span class="text-xs font-medium text-foreground">#{{ item.attempt }}</span>
                        <span class="text-xs text-muted-foreground">{{ formatFollowupDate(item.sent_at) }}</span>
                    </div>
                    <p class="mt-1 line-clamp-2 text-xs text-muted-foreground">{{ item.message_text }}</p>
                </div>
            </div>
            <p v-else class="text-xs text-muted-foreground">Sem historico de follow-up</p>
        </section>
        ...
        >
            ...
            <ul class="space-y-2 text-xs">
                <li v-for="(event, idx) in conversation.recentEvents" :key="idx" class="flex items-center justify-between gap-2">
                    <span class="text-foreground">{{ eventLabels[event.event_type] ?? event.event_type }}</span>
                    <span class="text-muted-foreground">{{ formatEventDate(event.created_at) }}</span>
                </li>
            </ul>
        </section>
        ...
        <Dialog :open="showClearConfirm" @update:open="(open) => { if (!open) showClearConfirm = false }">
            <DialogContent class="sm:max-w-md">
                <DialogHeader>
                    <DialogTitle>Limpar histórico?</DialogTitle>
                </DialogHeader>
                <p class="text-sm text-muted-foreground">
                    Mensagens, memória do agente e histórico de follow-up serão apagados. O lead, CPF e status são preservados.
                </p>
        ...
        <Dialog :open="showDeleteConfirm" @update:open="(open) => { if (!open) { showDeleteConfirm = false; deleteConfirmText = ''; } }">
            <DialogContent class="sm:max-w-md">
                <DialogHeader>
                    <DialogTitle>Excluir lead?</DialogTitle>
                </DialogHeader>
                <p class="text-sm text-muted-foreground">
                    O lead será removido (soft delete). Timeline, follow-ups e auditoria são preservados. Digite
                    <span class="font-mono font-semibold text-rose-500">EXCLUIR</span> para confirmar.
                </p>
                <input
                ...
                <DialogFooter class="gap-2 sm:gap-2">
                    ...
                    <button
                        type="button"
                        :disabled="deleteLeadForm.processing || deleteConfirmText.trim() !== 'EXCLUIR'"

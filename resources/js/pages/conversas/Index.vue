...
const breadcrumbs = computed<BreadcrumbItem[]>(() => {
    ...
    return [
        ...
        {
            title: props.activeConversation.lead.nome,
            href: `/conversas/${props.activeConversation.lead.id}`,
        },
    ];
});

const pageTitle = computed(
    () => props.activeConversation?.lead.nome ?? 'Conversas',
);
...
const tenantId = computed(
    () => (page.props.auth as any)?.user?.tenant_id as string | undefined,
);
...
const visibleFlash = computed(() =>
    flashMessage.value && flashMessage.value !== dismissedFlash.value
        ? flashMessage.value
        : null,
);
const visibleError = computed(() =>
    flashError.value && flashError.value !== dismissedError.value
        ? flashError.value
        : null,
);
...
function dismissError(): void {
    dismissedError.value = flashError.value;
}

watch(flashMessage, (next) => {
    if (next && next !== dismissedFlash.value) {
        setTimeout(() => {
            if (flashMessage.value === next) {
                dismissedFlash.value = next;
            }
        }, 5000);
    }
});
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div
            class="flex h-[calc(100svh-3.5rem)] min-h-0 flex-col overflow-hidden p-0 sm:h-[calc(100svh-4rem)] sm:p-4 lg:h-[calc(100svh-7.5rem)]"
        >
            <div
                v-if="visibleError"
                class="mb-3 flex items-start gap-2 rounded-lg border border-rose-200 bg-rose-50 px-4 py-2.5 text-sm text-rose-700 dark:border-rose-900/50 dark:bg-rose-900/20 dark:text-rose-400"
            >
                ...
                <button
                    type="button"
                    aria-label="Fechar"
                    class="rounded p-1 hover:bg-rose-100 dark:hover:bg-rose-900/40"
                    @click="dismissError"
                >
                    <X class="h-3.5 w-3.5" />
                </button>
            ...
            <div
                v-else-if="visibleFlash"
                class="mb-3 flex items-start gap-2 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-2.5 text-sm text-emerald-700 dark:border-emerald-900/50 dark:bg-emerald-900/20 dark:text-emerald-400"
            >
                ...
                <button
                    type="button"
                    aria-label="Fechar"
                    class="rounded p-1 hover:bg-emerald-100 dark:hover:bg-emerald-900/40"
                    @click="dismissFlash"
                >
                    <X class="h-3.5 w-3.5" />
                </button>
            ...
            >
                <div
                    ...
                    :class="activeConversation ? 'hidden lg:block' : 'block'"
                >
                    <ConversationSidebar
                        :leads="leads"
                        :filters="filters"
                ...
                <div
                    class="min-h-0 min-w-0 [&>section]:h-full"
                    :class="activeConversation ? 'block' : 'hidden lg:block'"
                >
                    <ConversationThread
            ...
            <Sheet
                ...
                @update:open="showMobileDetails = $event"
            >
                <SheetContent
                    side="right"
                    class="w-full p-0 sm:max-w-md xl:hidden"
                >
                    <div
                        v-if="activeConversation"
                        class="h-full overflow-hidden [&>aside]:h-full"
                    >
                        <LeadDetailsPanel
                            :key="`mobile-${activeConversation.lead.id}`"
                            :conversation="activeConversation"
                        />
                    </div>
                </SheetContent>

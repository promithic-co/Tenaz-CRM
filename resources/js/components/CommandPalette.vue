<script setup lang="ts">
import { router, usePage } from '@inertiajs/vue3';
import {
    Search,
    LayoutGrid,
    MessageSquare,
    Headphones,
    Bot,
    Smartphone,
    FlaskConical,
    Users,
} from 'lucide-vue-next';
import { ref, watch, computed, onMounted, onUnmounted } from 'vue';
import StatusBadge from '@/components/StatusBadge.vue';
import { Dialog, DialogContent } from '@/components/ui/dialog';
import { useBackofficeRoutes } from '@/composables/useBackofficeRoutes';
import type { Lead, Agent } from '@/types/models';

// Named `inertiaPage` because the template already binds `page` as the v-for
// item over `filteredPages`.
const inertiaPage = usePage();
const routes = useBackofficeRoutes();

const open = ref(false);
const query = ref('');
const loading = ref(false);
const results = ref<{ leads: Lead[]; agents: Agent[] }>({
    leads: [],
    agents: [],
});
let debounceTimer: ReturnType<typeof setTimeout> | null = null;

// Playground is platform-operator tooling behind the `super_admin` gate, so it
// is offered only to super-admins — everyone else would land on a 403.
const staticPages = computed(() => [
    { title: 'Dashboard', href: '/dashboard', icon: LayoutGrid },
    { title: 'Conversas', href: '/conversas', icon: MessageSquare },
    { title: 'Contatos', href: '/contatos', icon: Users },
    { title: 'Atendimentos', href: '/atendimentos', icon: Headphones },
    { title: 'Agentes', href: '/agentes', icon: Bot },
    { title: 'WhatsApp', href: '/whatsapp', icon: Smartphone },
    ...(inertiaPage.props.auth?.is_super_admin === true
        ? [
              {
                  title: 'Playground',
                  href: routes.playground(),
                  icon: FlaskConical,
              },
          ]
        : []),
]);

const filteredPages = computed(() => {
    if (!query.value) return staticPages.value;
    return staticPages.value.filter((p) =>
        p.title.toLowerCase().includes(query.value.toLowerCase()),
    );
});

watch(query, (value) => {
    if (debounceTimer) clearTimeout(debounceTimer);
    if (value.length < 2) {
        results.value = { leads: [], agents: [] };
        loading.value = false;
        return;
    }
    loading.value = true;
    debounceTimer = setTimeout(async () => {
        try {
            const res = await fetch(`/search?q=${encodeURIComponent(value)}`, {
                headers: {
                    Accept: 'application/json',
                    'X-Requested-With': 'XMLHttpRequest',
                },
            });
            results.value = await res.json();
        } finally {
            loading.value = false;
        }
    }, 300);
});

function handleKeydown(e: KeyboardEvent) {
    if ((e.ctrlKey || e.metaKey) && e.key === 'k') {
        e.preventDefault();
        open.value = !open.value;
    }
}

onMounted(() => window.addEventListener('keydown', handleKeydown));
onUnmounted(() => window.removeEventListener('keydown', handleKeydown));

function navigateTo(href: string) {
    open.value = false;
    query.value = '';
    router.visit(href);
}
</script>

<template>
    <Dialog v-model:open="open">
        <DialogContent class="max-w-lg gap-0 overflow-hidden p-0">
            <div class="flex items-center border-b px-3">
                <Search class="mr-2 h-4 w-4 shrink-0 text-muted-foreground" />
                <input
                    v-model="query"
                    autofocus
                    type="text"
                    placeholder="Buscar leads, agentes, páginas..."
                    class="flex-1 bg-transparent py-3 text-sm outline-none placeholder:text-muted-foreground"
                />
                <kbd
                    class="rounded border px-1 py-0.5 text-xs text-muted-foreground"
                    >Esc</kbd
                >
            </div>
            <div class="max-h-[400px] overflow-y-auto py-2">
                <div v-if="loading" class="flex justify-center py-6">
                    <div
                        class="h-5 w-5 animate-spin rounded-full border-2 border-primary border-t-transparent"
                    />
                </div>
                <template v-else>
                    <div v-if="results.leads.length > 0" class="px-2 pb-2">
                        <p
                            class="px-2 py-1 text-xs font-medium tracking-wide text-muted-foreground uppercase"
                        >
                            Leads
                        </p>
                        <button
                            v-for="lead in results.leads"
                            :key="lead.id"
                            @click="navigateTo(`/conversas/${lead.id}`)"
                            class="flex w-full items-center gap-3 rounded-md px-2 py-2 text-left text-sm hover:bg-accent"
                        >
                            <div class="min-w-0 flex-1">
                                <div class="truncate font-medium">
                                    {{ lead.nome ?? lead.whatsapp }}
                                </div>
                                <div
                                    class="truncate text-xs text-muted-foreground"
                                >
                                    {{ lead.whatsapp }}
                                </div>
                            </div>
                            <StatusBadge :status="lead.status" />
                        </button>
                    </div>
                    <div v-if="results.agents.length > 0" class="px-2 pb-2">
                        <p
                            class="px-2 py-1 text-xs font-medium tracking-wide text-muted-foreground uppercase"
                        >
                            Agentes
                        </p>
                        <button
                            v-for="agent in results.agents"
                            :key="agent.id"
                            @click="navigateTo(`/agentes/${agent.id}/config`)"
                            class="flex w-full items-center gap-3 rounded-md px-2 py-2 text-left text-sm hover:bg-accent"
                        >
                            <Bot
                                class="h-4 w-4 shrink-0 text-muted-foreground"
                            />
                            <span class="font-medium">{{ agent.name }}</span>
                        </button>
                    </div>
                    <div class="px-2">
                        <p
                            class="px-2 py-1 text-xs font-medium tracking-wide text-muted-foreground uppercase"
                        >
                            Páginas
                        </p>
                        <button
                            v-for="page in filteredPages"
                            :key="page.href"
                            @click="navigateTo(page.href)"
                            class="flex w-full items-center gap-3 rounded-md px-2 py-2 text-left text-sm hover:bg-accent"
                        >
                            <component
                                :is="page.icon"
                                class="h-4 w-4 shrink-0 text-muted-foreground"
                            />
                            <span>{{ page.title }}</span>
                        </button>
                    </div>
                    <div
                        v-if="
                            query.length >= 2 &&
                            !loading &&
                            results.leads.length === 0 &&
                            results.agents.length === 0
                        "
                        class="py-8 text-center text-sm text-muted-foreground"
                    >
                        Nenhum resultado para "{{ query }}"
                    </div>
                </template>
            </div>
        </DialogContent>
    </Dialog>
</template>

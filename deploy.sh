...
export_env_from_file() {
    local f="$1"
    local vars="APP_KEY APP_URL DB_HOST DB_PORT DB_DATABASE DB_USERNAME DB_PASSWORD REDIS_HOST REDIS_PORT OPENAI_API_KEY OPENROUTER_API_KEY CREDFLOW_API_KEY CREDFLOW_WEBHOOK_CONSULTA CREDFLOW_WEBHOOK_ESCALAR CREDFLOW_WEBHOOK_REGISTRAR REVERB_APP_ID REVERB_APP_KEY REVERB_APP_SECRET META_APP_ID META_APP_SECRET META_WEBHOOK_VERIFY_TOKEN META_APP_CONFIG_ID META_APP_CONFIG_ID_COEXISTENCE META_GRAPH_API_VERSION TENAZ_AGENT_FAILOVER_ENABLED TENAZ_AGENT_FAILOVER_PROVIDER TENAZ_AGENT_FAILOVER_MODEL"
    for key in $vars; do

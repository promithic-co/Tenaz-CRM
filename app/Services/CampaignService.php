...
class CampaignService
{
    ...
    public function resume(Campaign $campaign): void
    {
        DB::transaction(function () use ($campaign): void {
            ...
            // stopped — every campaign paused by the old failure-rate breaker would resume
            // still showing "Taxa de falha excedeu o limite", about a mechanism that no
            // longer exists. reopenForRevival already does this on the retry path.
            //
            // pause_reason_code deliberately survives: it drives the Meta account-block and
            // quality-risk banners, and those conditions are still true after a resume.
            $locked->update([
                'status' => 'sending',
                'paused_at' => null,
                'failure_reason' => null,
     ...
     *
     * @param  array{daily_limit: int, delay_between_ms: int}  $attributes
     */
    ...
    /**
     * Retry a single failed recipient. Deliberate, one-row operator action — there is no
     * bulk "reprocess every failure" counterpart, because most campaign failures are
     * malformed numbers (INVALID_PHONE) that re-fail identically forever. The bulk path is
     * to export the failed rows, fix the numbers in the spreadsheet, and load a new list.
     *
     ...
     * (may have reached the contact) and are owned by the reconciliation timeout. Scoped
    ...
    /*
     * There is deliberately no failure-rate auto-pause here any more.
     *
     * The percentage breaker used to pause a campaign once total_failed / total_sent passed
     * a per-campaign threshold. It was removed because it measured the wrong thing: the bulk
     * of campaign failures are malformed numbers in the imported list (INVALID_PHONE, caught
     * by SendCampaignMessageJob before the provider call, so they never reach Meta at all).
     * Those say nothing about account health, cost no reputation, and are not a reason to
     * stop a healthy campaign — one run was paused at 11% while delivering 95% and getting
     * a 90% reply rate.
     *
     * What actually protects the WhatsApp Business Account is unaffected and still pauses on
     * the FIRST occurrence, none of it routed through this class:
     *   - account blocked / payment (131031, 131042) — SendCampaignMessageJob and
     *     ProcessCampaignDeliveryEventJob, classified by MetaAccountErrorTaxonomy
     *   - Meta quality rating red — MetaQualityRiskService
     *   - wallet error 1003 — MonitorCampaignsCommand
     *
     * MonitorCampaignsCommand still raises a non-blocking 'high_failure_rate' alert, because
     * failure volume does feed the account's quality rating; it notifies, it never pauses.
     */

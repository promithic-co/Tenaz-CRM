    ...
    $this->actingAs($user)
        ...
        ->assertInertia(fn ($page) => $page
            ...
            ->has('leads.data.0', fn ($lead) => $lead
                ...
                ->has('pausado')
                ->has('is_returning')
                ->has('last_message_body')
                ->has('last_message_direction')
                ->has('awaiting_reply')
            )
            // inbox tab counters
            ->has('group_counts.fila')
            ->has('group_counts.minhas')
            ->has('group_counts.ia')
    ...
    $this->actingAs($user)
        ...
        ->assertInertia(fn ($page) => $page
            ...
            ->has('activeConversation', fn ($conv) => $conv
                ...
                // `followupHistory` and `recentEvents` were folded into `history`,
                // the single merged timeline the panel now renders.
                ->has('history.entries')

...
test('inbound agent turn writes unified timeline and queues whatsapp outbox', function () {
    ...
    expect(\App\Models\WhatsappOutboxMessage::query()->where('lead_id', $lead->id)->count())->toBe(1);
});
...
test('CRM-only lead is forced into manual mode regardless of instance default', function () {
    ...
    $automation = app(\App\Services\ConversationAutomationService::class);
...
test('duplicate webhook delivery with same provider_message_id is idempotent', function () {
    ...
    expect(\App\Models\ConversationTimelineMessage::query()
...
test('outbox processor marks timeline sent with provider id', function () {
    ...
    $user = User::factory()->create();
    $instance = \App\Models\WhatsappInstance::factory()->metaCloud()->for($user)->create([
        'name' => 'test-instance',
    ]);
    ...
    $timeline = app(\App\Services\ConversationTimelineService::class)->record(
        lead: $lead,
        direction: 'outbound',
        senderType: 'human',
        body: 'Mensagem',
    ...
    $outbox = \App\Models\WhatsappOutboxMessage::create([
        'tenant_id' => (string) $user->tenantId,
        'lead_id' => $lead->id,
        'channel' => 'whatsapp',
        'provider' => 'meta_cloud',
